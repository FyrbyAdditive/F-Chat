...
final class OAuthCoordinator {
    ...
    /// (`fyxlocal://oauth/callback`). Registered in the .app's Info.plist
    /// via make-app.sh; ASWebAuthenticationSession dispatches the redirect
    /// back to us automatically. The legacy `fchat://` scheme is ALSO
    /// registered in Info.plist (as an alias) so OAuth servers that still
    /// hold a previously-registered `fchat://` redirect keep working after
    /// the rename; new authorizations use the `fyxlocal://` URI below.
    static let redirectURI = "fyxlocal://oauth/callback"
    static let callbackURLScheme = "fyxlocal"
