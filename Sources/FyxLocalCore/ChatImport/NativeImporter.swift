...
/// Re-imports FyxLocal's own native JSON export (an array of `Conversation`
...
enum NativeImporter {
    ...
    static func looksLikeFyxLocal(_ json: Any) -> Bool {
        ...
        guard let messages = first["messages"] as? [[String: Any]] else {
            // A conversation with no messages is still valid FyxLocal JSON.
